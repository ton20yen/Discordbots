...
async def call(message, msg):
    ...
    team_battle_data = {}
    team_battle_data["member_num"] = 3
    team_battle_data['win'] = 2
    team_battle_data['waseda_flag'] = False
    team_battle_data["report_num"] = 3
    ...
    for i in range( int((((len(temp_users)/3)*10)+7)/10) ):
        if (len(users)-3) == 1:
            ...
            team_user_A = users.pop(0)
            team_user_B = users.pop(0)
            team_user_C = users.pop(0)
            team_name = team_user_A.display_name + "・" + team_user_B.display_name + "・" + team_user_C.display_name + "チーム"
            team_member.extend([team_user_A.display_name, team_user_B.display_name, team_user_C.display_name])
            team_member_id.extend([team_user_A.name, team_user_B.name, team_user_C.name])
            team_roll_call_count[team_name] = 3
